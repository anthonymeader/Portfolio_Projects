...
#define REG1_BASE_RATE_OFFSET 0x04
#define REG2_SYS_CLKS_OFFSET 0x08
...
/* Registers REG1 (Base_rate) and REG2 (SYS_CLKs_sec)                    */
	...
	struct device_attribute *attr, char *buf)
{
	...
	base_reg = ioread32(priv->base_addr + REG1_BASE_RATE_OFFSET);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	// See https://elixir.bootlin.com/linux/latest/source/lib/kstrtox.c#L289 
	ret = kstrtou8(buf, 0, &base_reg);
	if (ret < 0) {
		// kstrtou16 returned an error
		return ret;
	}

	iowrite32(base_reg, priv->base_addr + REG1_BASE_RATE_OFFSET);
...
-----------------------
*/
static ssize_t sys_clks_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	u32 sys_clk;
	// Get the private hps_led_patterns data out of the dev struct
	...
	sys_clk = ioread32(priv->base_addr + REG2_SYS_CLKS_OFFSET);

	return scnprintf(buf, PAGE_SIZE, "%u\n", sys_clk);
}

static ssize_t sys_clks_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size)
{
	u32 sys_clk;
	int ret;
	...
	// Parse the string we received as a u8
	// See https://elixir.bootlin.com/linux/latest/source/lib/kstrtox.c#L289 
	ret = kstrtou32(buf, 0, &sys_clk);
	if (ret < 0) {
		// kstrtou16 returned an error
		return ret;
	}

	iowrite32(sys_clk, priv->base_addr + REG2_SYS_CLKS_OFFSET);
...
static DEVICE_ATTR_RW(sys_clks);
...
static const struct of_device_id hps_led_patterns_of_match[] = {
	...
	{ .compatible = "ajm,HPS_LED_Patterns", },  
	{ }
};
